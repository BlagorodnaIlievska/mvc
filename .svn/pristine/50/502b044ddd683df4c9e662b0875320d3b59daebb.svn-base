...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			title: false,
			idProperty: 'id',
			pagestat: '{from} до {to} од {total}',
			pagetext: 'Страна',
			outof: 'од',
			findtext: 'Барај',
			params: [], //allow optional parameters to be passed around
			procmsg: 'Се процесира, ве молиме почекајте ...',
			query: '',
			qtype: '',
			nomsg: 'Нема резултати',
        ...
        g.hTable.className='flexigrid-tabela';
		...
		if (p.title) {
			...
			if (p.showTableToggleBtn) {
				$(g.mDiv).append('<div class="ptogtitle" title="Намали/Зголеми Табела"><span></span></div>');
		...
		if ($('th', g.hDiv).length) {
			...
			g.nDiv.innerHTML = "<table cellpadding='0' cellspacing='0' class='flexigrid-tabela'><tbody></tbody></table>";
			$(g.nDiv).css({
				marginBottom: (gh * -1),
				display: 'none',
				top: gtop
